...
export default function MoveDictionary({ visible }: Props) {
    const classes = ['card']
    const name = 'move-dictionary'
    const options: React.ReactElement[] = [<Option value='Select a Move' />]
    const [selectedTitle, setSelectedTitle] = useState('Select a Move')
    const [selectedContent, setSelectedContent] = useState<string[]>([])

    !visible && classes.push('hidden')
    Object.keys(moves).forEach((key) => options.push(<Option value={key} />))

    const handleSelectChange = (value: string) => {
        setSelectedTitle(value)
        setSelectedContent(moves[value as keyof typeof moves] || [])
    }

    return (
        <form name={name} className={classes.join(' ')}>
            <label form={name}>Move Dictionary</label>
            <Select 
                key='move-select' 
                id={name} 
                form={name} 
